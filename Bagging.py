# -*- coding: utf-8 -*-
"""
Created on Thu Apr 23 01:42:22 2020
...
test1 = DataFrame(data = [[1, 0.697, 0.460, 'notknown']], 
...
def calcMinGiniIndex(a, y, D):
    """
    Calculate the weighted Gini INDEX in a specific dataset after it is 
...
def predictSingle(tree, x):
    ...
    #the leaf node is not on first layer, but second, third, or fourth layer, 
...
def predictBase(tree, X):
    ...
    @X: DataFrame, each row is a sample and each column is a feature, the 
        ...
        The feature columns should be continuous values
    """    
    result = []
...
def BaggingPredict(X, trees):
    """
    Use the constructed Bagging model to predict sample labels
    @X: DataFrame, each row is a sample and each column is a feature, the 
    ...
    for tree in trees:
        agg_est += list(predictBase(tree = tree, X = X).\
                                map({'good':1, 'bad':-1}))
    ...
    results[list(agg_est < 0)] = -1
    results = results.map({1:'good', -1:'bad'})
    
    return results

def pltBaggingDecisionBound(X_, trees):
    """
    Draw the decision boundary from the Bagging model with C4.5 decision trees 
    as base learners
    ...
    #numpy.linspace(start, stop, num = 50) Return evenly spaced numbers overs a 
    #                                      specified interval.
    ...
    Z_ = BaggingPredict(X = DataFrame(np.c_[X_tmp.ravel(), Y_tmp.ravel()]), \
                        trees = trees).reshape(X_tmp.shape)
...
if __name__ == '__main__':
    trees, err_rates = BaggingTrain(X = watermelon, tree_num = 21, 
                                    treedeeplim = 0)
    
    print np.mean(BaggingPredict(X = watermelon, trees = trees) == \
                  watermelon.ix[:,watermelon.shape[1]-1])
    #Note, after the line continuation character "\", there should be NO BLANK

    pltBaggingDecisionBound(X_ = watermelon, trees = trees)
