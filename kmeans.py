# -*- coding: utf-8 -*-
"""
Created on Sun Jun 14 20:00:25 2020
...
class KMeans(object):
    ...
    #like self = KMeans(3) here, and then this self can be used to test other 
    #methods need to be written.
    
    def find_closest_centroids(self, X, centroid):
        """
        Return which clusters the samples in X belong to as an array with cluster 
        indeces for the samples (0 based number)
        ...
        #3 distances of each sample to the 3 centroids.
        ...
        #(3L, 2L), not 2 2-d arrays with a shape of (30L, 3L).
        ...
        #to the 3 centroids (3L).
        
        idx = distance.argmin(axis = 1)
        #np.argmin(array_like, axis) Returns the indices of the minimum values 
        #                            along an axis.
        #The shape of distance is (30L, 3L). The final result of 
        #distance.argmin(axis = 1) is 
        #array([2, 2, 2, 2, 1, 1, 1, 1, 2, 0, 
        #       0, 0, 2, 2, 1, 1, 2, 0, 0, 0, 
        #       2, 2, 1, 1, 1, 2, 1, 1, 2, 1], dtype = int64)
        #It means for each of the 30 samples (30L) included in distance, 
        #the minimum one of the 3 sample to centroid distances (3L), will be 
        #selected and its index will be returned. This index also indicates 
        #the cluster this sample belongs to.
    ...
    def compute_centroids(self, X, idx): 
        ...
        centroids.index = range(centroids.shape[0])
    ...
    def fit(self, X, initial_centroid_index = None, max_iters = 10, seed = 16, 
        ...
        if initial_centroid_index is None:
            np.random.seed(seed)
            initial_centroid_index = np.random.randint(1, m + 1, self.k)
